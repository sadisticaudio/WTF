...
namespace sadistic {
    ...
    struct RoutingMatrix : DragAndDropContainer, Component {
        ...
        RoutingMatrix(WaveTableManager& m) : mgmt(m) {
            
            EffectRoutingState tempFX[numProFX];
            ...
            std::sort(tempFX, tempFX + numProFX);
            for (int i { 0 }; i < numProFX; ++i) fx[tempFX[i].route].add(tempFX[i]);
            for (int j { 0 }; j < numRoutes; ++j) { fx[j].add({ nullptr, &endPoint[j] }); for(auto f : fx[j]) addAndMakeVisible(f.iPtr); }
            
            for (int i { 0 }; i < numProFX; ++i) { addAndMakeVisible(effects[i]); effects[i].addMouseListener(this, true); }
        ...
        Effect effects[numFX + numProFX] {
            { *this, getProFxID(0), mgmt.apvts },
            { *this, getProFxID(1), mgmt.apvts },
            { *this, getProFxID(2), mgmt.apvts },
            { *this, getProFxID(3), mgmt.apvts },
            { *this, getProFxID(4), mgmt.apvts },
            { *this, getProFxID(5), mgmt.apvts },
            { *this, getFxID(0), mgmt.apvts },
            { *this, getFxID(1), mgmt.apvts },
            { *this, getFxID(2), mgmt.apvts },
            { *this, getFxID(3), mgmt.apvts },
            { *this, getFxID(4), mgmt.apvts }
        };
        ...
        Component frames[numSliders + 2];
        EmpiricalSlider knobs[numSliders] { { true, true }, { false, true }, { true, true }, { false, true } };
        APVTS::SliderAttachment attachments[numSliders] {
            { mgmt.apvts, { "staticClipperDrive" },      knobs[0] },
            { mgmt.apvts, { "staticClipperDrive" },     knobs[1] },
            { mgmt.apvts, { "staticClipperDrive" },      knobs[2] },
            { mgmt.apvts, { "staticClipperDrive" },     knobs[3] }
        };
        SadLabel labels[2] { { "FILTER A", false, 0.f }, { "FILTER B", false, 0.f } };
    };
}
